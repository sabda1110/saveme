  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? 'bg-[#0f1117]/85 backdrop-blur-md border-b border-[#2d3348]/80 py-3 shadow-xl'
          : 'bg-transparent py-5'
      )}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              ...
              >
                {link.isHighlight && <Sparkles className="w-3.5 h-3.5 text-green-400 animate-pulse" />}
                {link.label}
          ...
          <div className="flex md:hidden items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-xl text-slate-300 hover:text-white hover:bg-[#21263a] border border-[#2d3348]"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-3 p-4 rounded-2xl bg-[#1a1d27] border border-[#2d3348] shadow-2xl flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-200">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                onClick={() => setMobileMenuOpen(false)}
                className="px-3 py-2 text-sm font-medium text-slate-200 hover:text-green-400 hover:bg-[#21263a] rounded-lg transition-colors flex items-center justify-between"
              >
                <span>{link.label}</span>
                {link.isHighlight && <Badge variant="brand" size="sm">Coba Sekarang</Badge>}
              </a>
            ))}
            <div className="pt-3 border-t border-[#2d3348] flex flex-col gap-2">
              {user ? (
